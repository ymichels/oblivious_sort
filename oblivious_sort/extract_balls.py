...
def extract_balls(conf: Config, arr: RAM, orginial_size):
    ...
    while read_position < arr.get_size():
        bin = arr.read_chunk((read_position, read_position + conf.BIN_SIZE))
        bin = [block for block in bin if block != RAM.dummey_note]
        random.shuffle(bin)
        result.write_chunk((write_position, write_position + len(bin)), bin)
